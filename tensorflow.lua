...
} static inline size_t TF_align16(size_t i) { return (i + 0xF) & ~0xF; } static inline size_t TF_max(size_t a, size_t b) { return a > b ? a : b; } static inline size_t TF_min(size_t a, size_t b) { return a < b ? a : b; } typedef enum TF_TString_Type { TF_TSTR_SMALL = 0x00, TF_TSTR_LARGE = 0x01, TF_TSTR_OFFSET = 0x02, TF_TSTR_VIEW = 0x03, TF_TSTR_TYPE_MASK = 0x03 } TF_TString_Type;
...
static inline TF_TString_Type TF_TString_GetType(const TF_TString *str) {
	return (TF_TString_Type)(str->u.raw.raw[0] & TF_TSTR_TYPE_MASK);
} static inline size_t TF_TString_ToActualSizeT(size_t size) {
	return size >> 2;
} static inline size_t TF_TString_ToInternalSizeT(size_t size, TF_TString_Type type) {
	return (size << 2) | type;
	...
	if (TF_TString_GetType(str) == TF_TSTR_LARGE && str->u.large.ptr != ((void *)0)) { free(str->u.large.ptr);
	TF_TString_Init(str);
} } static inline size_t TF_TString_GetSize(const TF_TString *str) {
	switch (TF_TString_GetType(str)) { case TF_TSTR_SMALL: return str->u.smll.size >> 2;
	case TF_TSTR_LARGE: return TF_TString_ToActualSizeT(str->u.large.size);
	case TF_TSTR_OFFSET: return str->u.offset.size >> 2;
	case TF_TSTR_VIEW: return TF_TString_ToActualSizeT(str->u.view.size);
	default: return 0;
} } static inline size_t TF_TString_GetCapacity(const TF_TString *str) {
	switch (TF_TString_GetType(str)) { case TF_TSTR_SMALL: return TF_TString_SmallCapacity;
	case TF_TSTR_LARGE: return str->u.large.cap;
	case TF_TSTR_OFFSET: case TF_TSTR_VIEW: default: return 0;
} } static inline const char *TF_TString_GetDataPointer(const TF_TString *str) {
	switch (TF_TString_GetType(str)) { case TF_TSTR_SMALL: return str->u.smll.str;
	case TF_TSTR_LARGE: return str->u.large.ptr;
	case TF_TSTR_OFFSET: return (const char *)str + str->u.offset.offset;
	case TF_TSTR_VIEW: return str->u.view.ptr;
	default: return ((void *)0);
} } static inline char *TF_TString_ResizeUninitialized(TF_TString *str, size_t new_size) {
	...
	str->u.smll.size = (uint8_t)((new_size << 2) | TF_TSTR_SMALL);
...
if (new_cap == curr_cap) {
	...
	new_ptr = (char *)realloc(str->u.large.ptr, new_cap + 1);
} else {
	new_ptr = (char *)malloc(new_cap + 1);
	if (copy_size) {
...
return str->u.large.ptr;
	...
	switch (TF_TString_GetType(str)) { case TF_TSTR_SMALL: return str->u.smll.str;
	case TF_TSTR_OFFSET: case TF_TSTR_VIEW: TF_TString_ResizeUninitialized(str, TF_TString_GetSize(str));
	return (TF_TString_GetType(str) == TF_TSTR_SMALL) ? str->u.smll.str : str->u.large.ptr;
	case TF_TSTR_LARGE: return str->u.large.ptr;
	default: return ((void *)0);
...
if (curr_type == TF_TSTR_LARGE) {
	str->u.large.ptr = (char *)realloc(str->u.large.ptr, new_cap + 1);
} else {
	char *new_ptr = (char *)malloc(new_cap + 1);
...
} static inline void TF_TString_ReserveAmortized(TF_TString *str, size_t new_cap) {
	const size_t curr_cap = TF_TString_GetCapacity(str);
	if (new_cap > curr_cap) {
	TF_TString_Reserve(str, new_cap > 2 * curr_cap ? new_cap : 2 * curr_cap);
} } static inline char *TF_TString_Resize(TF_TString *str, size_t new_size, char c) {
	size_t curr_size = TF_TString_GetSize(str);
	char *cstr = TF_TString_ResizeUninitialized(str, new_size);
	if (new_size > curr_size) {
	memset(cstr + curr_size, c, new_size - curr_size);
} return cstr;
} static inline void TF_TString_AssignView(TF_TString *dst, const char *src, size_t size) {
	TF_TString_Dealloc(dst);
	dst->u.view.size = TF_TString_ToInternalSizeT(size, TF_TSTR_VIEW);
	dst->u.view.ptr = src;
} static inline void TF_TString_AppendN(TF_TString *dst, const char *src, size_t src_size) {
	if (!src_size) return;
...
} static inline void TF_TString_Copy(TF_TString *dst, const char *src, size_t size) {
	...
	case TF_TSTR_SMALL: case TF_TSTR_VIEW: *dst = *src;
	return;
	case TF_TSTR_LARGE: {
	const char *src_c = TF_TString_GetDataPointer(src);
	size_t size = TF_TString_GetSize(src);
