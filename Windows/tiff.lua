...
/* #define TIFFLIB_VERSION_STR "LIBTIFF, Version 4.2.0\nCopyright (c) 1988-1996 Sam Leffler\nCopyright (c) 1991-1996 Silicon Graphics, Inc." ### string, not number "\"LIBTIFF, Version 4.2.0\\nCopyright (c) 1988-1996 Sam Leffler\\nCopyright (c) 1991-1996 Silicon Graphics, Inc.\"" */
...
typedef signed long long tmsize_t;
...
wrapper = require 'ffi.libwrapper'{
	...
	defs = {
		...
		TIFFGetVersion = [[const char* TIFFGetVersion(void);]],
		TIFFFindCODEC = [[const TIFFCodec* TIFFFindCODEC(uint16);]],
		TIFFRegisterCODEC = [[TIFFCodec* TIFFRegisterCODEC(uint16, const char*, TIFFInitMethod);]],
		TIFFUnRegisterCODEC = [[void TIFFUnRegisterCODEC(TIFFCodec*);]],
		TIFFIsCODECConfigured = [[int TIFFIsCODECConfigured(uint16);]],
		TIFFGetConfiguredCODECs = [[TIFFCodec* TIFFGetConfiguredCODECs(void);]],
		...
		TIFFGetTagListEntry = [[uint32 TIFFGetTagListEntry( TIFF *, int tag_index );]],
		TIFFFindField = [[const TIFFField* TIFFFindField(TIFF *, uint32, TIFFDataType);]],
		TIFFFieldWithTag = [[const TIFFField* TIFFFieldWithTag(TIFF*, uint32);]],
		TIFFFieldWithName = [[const TIFFField* TIFFFieldWithName(TIFF*, const char *);]],
		TIFFFieldTag = [[uint32 TIFFFieldTag(const TIFFField*);]],
		...
		TIFFFieldWriteCount = [[int TIFFFieldWriteCount(const TIFFField*);]],
		...
		TIFFGetField = [[int TIFFGetField(TIFF* tif, uint32 tag, ...);]],
		TIFFVGetField = [[int TIFFVGetField(TIFF* tif, uint32 tag, va_list ap);]],
		TIFFGetFieldDefaulted = [[int TIFFGetFieldDefaulted(TIFF* tif, uint32 tag, ...);]],
		TIFFVGetFieldDefaulted = [[int TIFFVGetFieldDefaulted(TIFF* tif, uint32 tag, va_list ap);]],
		TIFFReadDirectory = [[int TIFFReadDirectory(TIFF* tif);]],
		TIFFReadCustomDirectory = [[int TIFFReadCustomDirectory(TIFF* tif, toff_t diroff, const TIFFFieldArray* infoarray);]],
		...
		TIFFScanlineSize64 = [[uint64 TIFFScanlineSize64(TIFF* tif);]],
		TIFFScanlineSize = [[tmsize_t TIFFScanlineSize(TIFF* tif);]],
		TIFFRasterScanlineSize64 = [[uint64 TIFFRasterScanlineSize64(TIFF* tif);]],
		TIFFRasterScanlineSize = [[tmsize_t TIFFRasterScanlineSize(TIFF* tif);]],
		TIFFStripSize64 = [[uint64 TIFFStripSize64(TIFF* tif);]],
		TIFFStripSize = [[tmsize_t TIFFStripSize(TIFF* tif);]],
		TIFFRawStripSize64 = [[uint64 TIFFRawStripSize64(TIFF* tif, uint32 strip);]],
		TIFFRawStripSize = [[tmsize_t TIFFRawStripSize(TIFF* tif, uint32 strip);]],
		TIFFVStripSize64 = [[uint64 TIFFVStripSize64(TIFF* tif, uint32 nrows);]],
		TIFFVStripSize = [[tmsize_t TIFFVStripSize(TIFF* tif, uint32 nrows);]],
		TIFFTileRowSize64 = [[uint64 TIFFTileRowSize64(TIFF* tif);]],
		TIFFTileRowSize = [[tmsize_t TIFFTileRowSize(TIFF* tif);]],
		TIFFTileSize64 = [[uint64 TIFFTileSize64(TIFF* tif);]],
		TIFFTileSize = [[tmsize_t TIFFTileSize(TIFF* tif);]],
		TIFFVTileSize64 = [[uint64 TIFFVTileSize64(TIFF* tif, uint32 nrows);]],
		TIFFVTileSize = [[tmsize_t TIFFVTileSize(TIFF* tif, uint32 nrows);]],
		TIFFDefaultStripSize = [[uint32 TIFFDefaultStripSize(TIFF* tif, uint32 request);]],
		TIFFDefaultTileSize = [[void TIFFDefaultTileSize(TIFF*, uint32*, uint32*);]],
		...
		TIFFCurrentRow = [[uint32 TIFFCurrentRow(TIFF*);]],
		TIFFCurrentDirectory = [[uint16 TIFFCurrentDirectory(TIFF*);]],
		TIFFNumberOfDirectories = [[uint16 TIFFNumberOfDirectories(TIFF*);]],
		TIFFCurrentDirOffset = [[uint64 TIFFCurrentDirOffset(TIFF*);]],
		TIFFCurrentStrip = [[uint32 TIFFCurrentStrip(TIFF*);]],
		TIFFCurrentTile = [[uint32 TIFFCurrentTile(TIFF* tif);]],
		...
		TIFFWriteCheck = [[int TIFFWriteCheck(TIFF*, int, const char *);]],
		TIFFFreeDirectory = [[void TIFFFreeDirectory(TIFF*);]],
		TIFFCreateDirectory = [[int TIFFCreateDirectory(TIFF*);]],
		TIFFCreateCustomDirectory = [[int TIFFCreateCustomDirectory(TIFF*,const TIFFFieldArray*);]],
		...
		TIFFSetDirectory = [[int TIFFSetDirectory(TIFF*, uint16);]],
		TIFFSetSubDirectory = [[int TIFFSetSubDirectory(TIFF*, uint64);]],
		TIFFUnlinkDirectory = [[int TIFFUnlinkDirectory(TIFF*, uint16);]],
		TIFFSetField = [[int TIFFSetField(TIFF*, uint32, ...);]],
		TIFFVSetField = [[int TIFFVSetField(TIFF*, uint32, va_list);]],
		TIFFUnsetField = [[int TIFFUnsetField(TIFF*, uint32);]],
		TIFFWriteDirectory = [[int TIFFWriteDirectory(TIFF *);]],
		TIFFWriteCustomDirectory = [[int TIFFWriteCustomDirectory(TIFF *, uint64 *);]],
		...
		TIFFReadScanline = [[int TIFFReadScanline(TIFF* tif, void* buf, uint32 row, uint16 sample);]],
		TIFFWriteScanline = [[int TIFFWriteScanline(TIFF* tif, void* buf, uint32 row, uint16 sample);]],
		TIFFReadRGBAImage = [[int TIFFReadRGBAImage(TIFF*, uint32, uint32, uint32*, int);]],
		TIFFReadRGBAImageOriented = [[int TIFFReadRGBAImageOriented(TIFF*, uint32, uint32, uint32*, int, int);]],
		TIFFReadRGBAStrip = [[int TIFFReadRGBAStrip(TIFF*, uint32, uint32 * );]],
		TIFFReadRGBATile = [[int TIFFReadRGBATile(TIFF*, uint32, uint32, uint32 * );]],
		TIFFReadRGBAStripExt = [[int TIFFReadRGBAStripExt(TIFF*, uint32, uint32 *, int stop_on_error );]],
		TIFFReadRGBATileExt = [[int TIFFReadRGBATileExt(TIFF*, uint32, uint32, uint32 *, int stop_on_error );]],
		...
		TIFFRGBAImageGet = [[int TIFFRGBAImageGet(TIFFRGBAImage*, uint32*, uint32, uint32);]],
		...
		TIFFComputeTile = [[uint32 TIFFComputeTile(TIFF* tif, uint32 x, uint32 y, uint32 z, uint16 s);]],
		TIFFCheckTile = [[int TIFFCheckTile(TIFF* tif, uint32 x, uint32 y, uint32 z, uint16 s);]],
		TIFFNumberOfTiles = [[uint32 TIFFNumberOfTiles(TIFF*);]],
		TIFFReadTile = [[tmsize_t TIFFReadTile(TIFF* tif, void* buf, uint32 x, uint32 y, uint32 z, uint16 s);]],
		TIFFWriteTile = [[tmsize_t TIFFWriteTile(TIFF* tif, void* buf, uint32 x, uint32 y, uint32 z, uint16 s);]],
		TIFFComputeStrip = [[uint32 TIFFComputeStrip(TIFF*, uint32, uint16);]],
		TIFFNumberOfStrips = [[uint32 TIFFNumberOfStrips(TIFF*);]],
		TIFFReadEncodedStrip = [[tmsize_t TIFFReadEncodedStrip(TIFF* tif, uint32 strip, void* buf, tmsize_t size);]],
		TIFFReadRawStrip = [[tmsize_t TIFFReadRawStrip(TIFF* tif, uint32 strip, void* buf, tmsize_t size);]],
		TIFFReadEncodedTile = [[tmsize_t TIFFReadEncodedTile(TIFF* tif, uint32 tile, void* buf, tmsize_t size);]],
		TIFFReadRawTile = [[tmsize_t TIFFReadRawTile(TIFF* tif, uint32 tile, void* buf, tmsize_t size);]],
		TIFFReadFromUserBuffer = [[int TIFFReadFromUserBuffer(TIFF* tif, uint32 strile, void* inbuf, tmsize_t insize, void* outbuf, tmsize_t outsize);]],
		TIFFWriteEncodedStrip = [[tmsize_t TIFFWriteEncodedStrip(TIFF* tif, uint32 strip, void* data, tmsize_t cc);]],
		TIFFWriteRawStrip = [[tmsize_t TIFFWriteRawStrip(TIFF* tif, uint32 strip, void* data, tmsize_t cc);]],
		TIFFWriteEncodedTile = [[tmsize_t TIFFWriteEncodedTile(TIFF* tif, uint32 tile, void* data, tmsize_t cc);]],
		TIFFWriteRawTile = [[tmsize_t TIFFWriteRawTile(TIFF* tif, uint32 tile, void* data, tmsize_t cc);]],
		TIFFDataWidth = [[int TIFFDataWidth(TIFFDataType);]],
		TIFFSetWriteOffset = [[void TIFFSetWriteOffset(TIFF* tif, toff_t off);]],
		TIFFSwabShort = [[void TIFFSwabShort(uint16*);]],
		TIFFSwabLong = [[void TIFFSwabLong(uint32*);]],
		TIFFSwabLong8 = [[void TIFFSwabLong8(uint64*);]],
		TIFFSwabFloat = [[void TIFFSwabFloat(float*);]],
		TIFFSwabDouble = [[void TIFFSwabDouble(double*);]],
		TIFFSwabArrayOfShort = [[void TIFFSwabArrayOfShort(uint16* wp, tmsize_t n);]],
		TIFFSwabArrayOfTriples = [[void TIFFSwabArrayOfTriples(uint8* tp, tmsize_t n);]],
		TIFFSwabArrayOfLong = [[void TIFFSwabArrayOfLong(uint32* lp, tmsize_t n);]],
		TIFFSwabArrayOfLong8 = [[void TIFFSwabArrayOfLong8(uint64* lp, tmsize_t n);]],
		...
		TIFFReverseBits = [[void TIFFReverseBits(uint8* cp, tmsize_t n);]],
		TIFFGetBitRevTable = [[const unsigned char* TIFFGetBitRevTable(int);]],
		TIFFGetStrileOffset = [[uint64 TIFFGetStrileOffset(TIFF *tif, uint32 strile);]],
		TIFFGetStrileByteCount = [[uint64 TIFFGetStrileByteCount(TIFF *tif, uint32 strile);]],
		TIFFGetStrileOffsetWithErr = [[uint64 TIFFGetStrileOffsetWithErr(TIFF *tif, uint32 strile, int *pbErr);]],
		TIFFGetStrileByteCountWithErr = [[uint64 TIFFGetStrileByteCountWithErr(TIFF *tif, uint32 strile, int *pbErr);]],
		LogL16toY = [[double LogL16toY(int);]],
		LogL10toY = [[double LogL10toY(int);]],
		XYZtoRGB24 = [[void XYZtoRGB24(float*, uint8*);]],
		uv_decode = [[int uv_decode(double*, double*, int);]],
		LogLuv24toXYZ = [[void LogLuv24toXYZ(uint32, float*);]],
		LogLuv32toXYZ = [[void LogLuv32toXYZ(uint32, float*);]],
		LogL16fromY = [[int LogL16fromY(double, int);]],
		LogL10fromY = [[int LogL10fromY(double, int);]],
		uv_encode = [[int uv_encode(double, double, int);]],
		LogLuv24fromXYZ = [[uint32 LogLuv24fromXYZ(float*, int);]],
		LogLuv32fromXYZ = [[uint32 LogLuv32fromXYZ(float*, int);]],
		TIFFCIELabToRGBInit = [[int TIFFCIELabToRGBInit(TIFFCIELabToRGB*, const TIFFDisplay *, float*);]],
		TIFFCIELabToXYZ = [[void TIFFCIELabToXYZ(TIFFCIELabToRGB *, uint32, int32, int32, float *, float *, float *);]],
		TIFFXYZToRGB = [[void TIFFXYZToRGB(TIFFCIELabToRGB *, float, float, float, uint32 *, uint32 *, uint32 *);]],
		TIFFYCbCrToRGBInit = [[int TIFFYCbCrToRGBInit(TIFFYCbCrToRGB*, float*, float*);]],
		TIFFYCbCrtoRGB = [[void TIFFYCbCrtoRGB(TIFFYCbCrToRGB *, uint32, int32, int32, uint32 *, uint32 *, uint32 *);]],
		TIFFMergeFieldInfo = [[int TIFFMergeFieldInfo(TIFF*, const TIFFFieldInfo[], uint32);]],
	},
}
